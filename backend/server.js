...
// Define endpoint to fetch pots
app.get("/posts", async (req , res)=>{
    try {

        const responce = await fetch("https://jsonplaceholder.typicode.com/posts?_limit=20");
        const data = await responce.json();
        res.json(data);
    } catch (error) {
        console.error("Error fetching pots:", error);
        res.status(500).json({ error: "Error fetching pots:"});
    }
});
